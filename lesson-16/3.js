...
class Worker {
    ...
    /* Есть ли смысл делать приватные "методы"?
    *  Ведь по сути, они будут в каждом экземпляре,
    *  а стандратный будет создан в прототипе, но
    *  к нему будет деоступ.
    */
    #numberValidate = (data) => {
        if(typeof data !== 'number'){
            throw new Error(`Rate should be a number, not ${typeof data}`);
        }
    };
    ...
    setRate(rate){
        this.#numberValidate(rate);

        this.#rate = rate;
    }

    setDays(days){
        this.#numberValidate(days);
